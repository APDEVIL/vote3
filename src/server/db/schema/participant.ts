...
// ── Relations (CRITICAL: Fixes 'referencedTable' error) ───────────────────────

export const participationRequestRelations = relations(participationRequest, ({ one }) => ({
    poll: one(poll, {
        fields: [participationRequest.pollId],
        references: [poll.id],
    }),
    voter: one(user, {
        fields: [participationRequest.userId],
        references: [user.id],
    }),
    reviewer: one(user, {
        fields: [participationRequest.reviewedBy],
        references: [user.id],
    }),
}));
